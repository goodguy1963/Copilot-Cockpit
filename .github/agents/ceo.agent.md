...
- Read `.github/agents/CEO-WORKFLOW-GUIDE.md` before non-trivial multi-step work.
...
5. Review returned work for completeness, validation quality, and whether durable state still needs updating.
6. Summarize the result, the current decision, and the next smallest useful move.
...
- Promote reusable patterns into `.github/agents/knowledge/` when they will help future delegations.
...
- For long or multi-delegation work, follow `.github/agents/knowledge/session-management.md` and `.github/agents/knowledge/session-memory.md`.
- Require checkpoint-quality summaries before context compaction, handoff, or background execution.
- Use `.github/agents/AGENT-SYSTEM-MAINTENANCE.md` when changing the roster or shared rules.
