...
const tempDir = path.join(os.tmpdir(), `cockpit-probe-${Date.now()}`);
...
if (fs.existsSync(privateJsonPath)) {
    ...
    console.log('scheduler.private.json NOT FOUND (expected if storageMode is sqlite and write was successful but chose not to create it)');
}