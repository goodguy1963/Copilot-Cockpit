...
(async () => {
    const tempDir = path.join(os.tmpdir(), 'cockpit-probe-workspace-' + Date.now());
    fs.mkdirSync(tempDir, { recursive: true });
    const vscodeDir = path.join(tempDir, '.vscode');
    fs.mkdirSync(vscodeDir);

    // 1. Create .vscode/settings.json - and set to sqlite as requested
    ...
    console.log('Testing with workspace out/server.js (sqlite mode)...');
