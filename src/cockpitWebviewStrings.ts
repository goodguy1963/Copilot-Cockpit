...
export function buildSchedulerWebviewStrings(
  ...
  return {
    ...
    telegramBotTokenHelp: localize(
      "Stored only in VS Code SecretStorage. Leave blank to keep the currently saved token.",
      "VS Code SecretStorage にのみ保存されます。現在保存されている token を維持する場合は空欄のままにしてください。",
      "Wird nur in VS Code SecretStorage gespeichert. Leer lassen, um das aktuell gespeicherte token beizubehalten.",
    ),
    ...
    telegramWorkspaceNote: localize(
      "The hook files are generated under .github/hooks and use VS Code SecretStorage for bot tokens.",
      "hook ファイルは .github/hooks 配下に生成され、bot token は VS Code SecretStorage を使用します。",
      "Die hook-Dateien werden unter .github/hooks erzeugt und verwenden VS Code SecretStorage für Bot-Token.",
    ),
    ...
    approvalModeDescription: localize(
      ...
      "この拡張機能が開始する新しい Copilot チャットセッションのデフォルト承認レベルと、レガシー YOLO フォールバックを制御します。",
      "Steuert die Standard-Genehmigungsstufe für neue Copilot-Chat-Sitzungen, die von dieser Erweiterung gestartet werden, einschließlich Legacy-YOLO-Fallback.",
    ),
    ...
    reviewDefaultsSaved: localize(
      "Needs-bot-review inserts the handoff comment and launches the planning prompt after save; ready uses its own prompt when creating a task draft. Any bundled-agent sync remains a separate manual Settings action.",
      "needs-bot-review は保存後に引き継ぎコメントを挿入して planning prompt を実行し、ready はタスク下書き作成時に専用プロンプトを使います。bundled-agent sync は引き続き Settings での別個の手動操作です。",
      "Needs-bot-review fügt nach dem Speichern den Übergabekommentar ein und startet den Planungsprompt; ready verwendet seinen eigenen Prompt beim Erstellen eines Task-Drafts. Jede Synchronisierung gebündelter Agenten bleibt eine separate manuelle Settings-Aktion.",
    ),
    ...
    settingsSearchProviderBody: localize(
      "Choose which lightweight search provider review prompts should reference for quick external lookups. When a request already includes a URL, built-in tools run first to keep API calls low.",
      "review プロンプトが素早い外部検索に使う、軽量検索プロバイダーを選びます。request に URL が含まれている場合は、API 呼び出しを抑えるために組み込みツールが先に使われます。",
      "Waehlen Sie, welchen leichten Suchanbieter Review-Prompts fuer schnelle externe Suchen verwenden sollen. Wenn eine Anfrage bereits eine URL enthaelt, werden zuerst integrierte Tools verwendet, damit moeglichst wenige API-Aufrufe noetig sind.",
    ),
    settingsSearchProviderHint: localize(
      "Built-in uses editor tools first and is the lowest extra-cost path. Tavily needs external MCP/API setup.",
      "built-in は editor の組み込みツールを先に使う、追加コストが最も低い経路です。Tavily は外部 MCP/API の設定が必要です。",
      "Built-in nutzt zuerst Editor-Werkzeuge und ist der Weg mit den geringsten Zusatzkosten. Tavily braucht eine externe MCP/API-Einrichtung.",
    ),
    ...
    settingsResearchProviderBody: localize(
      "Choose which deeper external research provider review prompts should reference after built-in URL checks, when extended grounding is still needed.",
      "review プロンプトが、組み込みの URL 確認の後でも追加の grounded research が必要な場合に使う research プロバイダーを選びます。",
      "Waehlen Sie, welchen externen Research-Anbieter Review-Prompts nach integrierten URL-Pruefungen verwenden sollen, wenn weiterhin tiefergehende fundierte Recherche noetig ist.",
    ),
    settingsResearchProviderHint: localize(
      "None keeps research on built-in/local tooling only. Perplexity, Tavily, and Google grounded research need external setup. URL-first checks still run before external research.",
      "None は research を組み込み/ローカルのツールだけに保ちます。Perplexity、Tavily、Google grounded research は外部設定が必要です。URL-first の確認は外部 research より前に続けて実行されます。",
      "None beschraenkt Recherche auf integrierte/lokale Werkzeuge. Perplexity, Tavily und Google Grounded Research brauchen eine externe Einrichtung. URL-first-Pruefungen laufen weiterhin vor externer Recherche.",
    ),
    ...
    boardKanbanDoneBlocked: localize("Only scheduled or final-review todos can be completed.", "Scheduledまたは最終レビュー中のTodoのみ完了できます。", "Nur geplante oder final geprüfte Todos können abgeschlossen werden."),
    boardKanbanUnknownLane: localize("Unknown Kanban lane.", "不明なKanbanレーンです。", "Unbekannte Kanban-Lane."),
    boardKanbanTodoMissing: localize("Todo Cockpit item not found.", "Todo Cockpit項目が見つかりません。", "Todo Cockpit-Eintrag nicht gefunden."),
