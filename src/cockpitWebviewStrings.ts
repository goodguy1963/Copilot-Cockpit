...
export function buildSchedulerWebviewStrings(
  ...
  return {
    ...
    reviewDefaultsDescription: localize(
      "Configure the repo-local handoff text and prompt templates for needs-bot-review and ready Todo workflows.",
      "needs-bot-review と ready の Todo ワークフローで使う、リポジトリローカルの引き継ぎテキストとプロンプトテンプレートを設定します。",
      "Konfigurieren Sie die repository-lokalen Übergabetexte und Prompt-Vorlagen für die Todo-Workflows needs-bot-review und ready.",
    ),
    ...
    reviewDefaultsBotPromptPlaceholder: localize(
      "Use {{todo_context}} and {{mcp_skill_guidance}} for the full handoff, or {{title}}, {{description}}, {{labels}}, {{recent_comments}}, and {{linked_task}} for individual fields.",
      "完全な引き継ぎには {{todo_context}} と {{mcp_skill_guidance}} を使い、個別項目には {{title}}、{{description}}、{{labels}}、{{recent_comments}}、{{linked_task}} を使えます。",
      "Verwenden Sie {{todo_context}} und {{mcp_skill_guidance}} für die vollständige Übergabe oder {{title}}, {{description}}, {{labels}}, {{recent_comments}} und {{linked_task}} für einzelne Felder.",
    ),
    ...
    reviewDefaultsReadyPromptPlaceholder: localize(
      "Use {{todo_context}} and {{mcp_skill_guidance}} so ready task drafts carry the full Todo handoff and the repo's MCP/skill usage guidance.",
      "ready のタスク下書きには {{todo_context}} と {{mcp_skill_guidance}} を使い、Todo の完全な引き継ぎとリポジトリの MCP / skill 利用方針を含めます。",
      "Verwenden Sie {{todo_context}} und {{mcp_skill_guidance}}, damit Ready-Task-Drafts die vollständige Todo-Übergabe und die MCP-/Skill-Nutzungshinweise des Repositorys enthalten.",
    ),
    reviewDefaultsSave: localize("Save Workflow Prompts", "ワークフロープロンプトを保存", "Workflow-Prompts speichern"),
    reviewDefaultsSaved: localize(
      "Needs-bot-review inserts the handoff comment and launches the planning prompt after save; ready uses its own prompt when creating a task draft.",
      "needs-bot-review は保存後に引き継ぎコメントを挿入して planning prompt を実行し、ready はタスク下書き作成時に専用プロンプトを使います。",
      "Needs-bot-review fügt nach dem Speichern den Übergabekommentar ein und startet den Planungsprompt; ready verwendet seinen eigenen Prompt beim Erstellen eines Task-Drafts.",
    ),
    ...
    settingsSupportBody: localize(
      "Configure repo-local Copilot MCP, skills, and starter agents here. Starter agents are useful when you want the orchestrator to hand bounded work to specialists instead of bloating one chat session with every intermediate step. Existing repo-local agent setups stay user-owned, bundled agents only sync when you approve a manual action, .github is backed up first when available, and customized workspace copies are skipped so your edits stay intact.",
      "ここで repo-local の Copilot 用 MCP、スキル、スターターエージェントを設定します。スターターエージェントは、orchestrator がすべてを 1 つの長いチャットに詰め込む代わりに、制限付きの作業を specialist へ渡したいときに役立ちます。既存の repo-local agent setup は引き続きユーザー所有として扱われ、同梱エージェントは手動操作を承認したときだけ同期され、.github があれば先にバックアップされ、ワークスペース側で変更したコピーはスキップされるため編集内容が保護されます。",
      "Konfigurieren Sie hier repo-lokales Copilot-MCP, Skills und Starter-Agenten. Starter-Agenten sind nützlich, wenn der Orchestrator begrenzte Arbeit an Spezialisten übergeben soll, statt einen einzelnen Chat mit allen Zwischenschritten aufzublähen. Bestehende repo-lokale Agent-Setups bleiben benutzereigen, gebündelte Agenten werden nur synchronisiert, wenn Sie eine manuelle Aktion freigeben, .github wird vorher gesichert, wenn vorhanden, und angepasste Workspace-Kopien werden übersprungen, damit Ihre Änderungen erhalten bleiben.",
    ),
