...
export function buildSchedulerWebviewStrings(
  ...
  return {
    ...
    helpAgentEcosystemBody: localize(
      "Learn the default workflow first: Todo for planning, Research when context is missing, then Task or Job for execution. After that, use Plan Integration if you want optional repo-local skills, prompts, MCP tools, or starter agents as a control-plane extension. Treat existing repo-local agent systems as user-owned, and only approve bundled-agent install or sync when you want a manual Settings action.",
      "まず標準ワークフローを理解してください。計画には Todo、文脈不足には Research、実行には Task または Job を使います。その後で必要なら Plan Integration を使い、repo-local の skills、prompts、MCP tools、starter agents を任意の制御プレーン拡張として追加します。既存の repo-local agent system はユーザー所有として扱い、bundled agents の install や sync は Settings からの手動操作を明示的に行うときだけ承認してください。",
      "Lernen Sie zuerst den Standardablauf: Todo fuer Planung, Research bei fehlendem Kontext und danach Task oder Job fuer die Ausfuehrung. Nutzen Sie erst dann Plan Integration, wenn Sie optionale repo-lokale Skills, Prompts, MCP-Tools oder Starter-Agenten als Control-Plane-Erweiterung wollen. Behandeln Sie bestehende repo-lokale Agentensysteme als benutzereigen und geben Sie die Installation oder Synchronisierung gebuendelter Agenten nur frei, wenn Sie eine manuelle Aktion in den Einstellungen wollen.",
    ),
    ...
    actionSyncBundledAgents: localize("Sync Bundled Agents", "同梱エージェントを同期", "Gebündelte Agenten synchronisieren"),
    ...
    settingsSupportBody: localize(
      "Configure repo-local Copilot MCP, skills, and starter agents here. Starter agents are useful when you want the orchestrator to hand bounded work to specialists instead of bloating one chat session with every intermediate step. Existing repo-local agent systems stay user-owned, bundled agent sync remains a manual Settings action so those systems are not overwritten, .github is backed up first when available, and customized workspace copies are skipped so your edits stay intact.",
      "ここで repo-local の Copilot 用 MCP、スキル、スターターエージェントを設定します。スターターエージェントは、orchestrator がすべてを 1 つの長いチャットに詰め込む代わりに、制限付きの作業を specialist へ渡したいときに役立ちます。既存の repo-local agent system は引き続きユーザー所有として扱われ、それらを上書きしないよう bundled agent sync は Settings での手動操作のまま維持され、.github があれば先にバックアップされ、ワークスペース側で変更したコピーはスキップされるため編集内容が保護されます。",
      "Konfigurieren Sie hier repo-lokales Copilot-MCP, Skills und Starter-Agenten. Starter-Agenten sind nützlich, wenn der Orchestrator begrenzte Arbeit an Spezialisten übergeben soll, statt einen einzelnen Chat mit allen Zwischenschritten aufzublähen. Bestehende repo-lokale Agent-Systeme bleiben benutzereigen, die Synchronisierung gebündelter Agenten bleibt eine manuelle Settings-Aktion, damit diese Systeme nicht überschrieben werden, .github wird vorher gesichert, wenn vorhanden, und angepasste Workspace-Kopien werden übersprungen, damit Ihre Änderungen erhalten bleiben.",
    ),
