...
export function buildSchedulerWebviewStrings(
  ...
  return {
    ...
    helpAgentEcosystemBody: localize(
      "Use the intro tutorial to learn the plugin first, then use plan integration to inspect the repo's current agent surfaces and design repo-local skills, prompts, and agent roles around the Todo communication hub and scheduler MCP tools. Treat existing repo-local agent systems as user-owned, do not install or sync bundled agents until you explicitly approve it, and if you later approve implementation, back up .github first when available before applying the agreed setup safely.",
      "まず Intro Tutorial でプラグインの全体像を理解し、その後 Plan Integration でリポジトリ内の既存エージェント構成を確認しながら、Todo コミュニケーションハブと scheduler MCP ツールを中心に repo-local のスキル、プロンプト、エージェント役割を設計します。既存の repo-local agent system はユーザー所有として扱い、明示的な承認があるまで bundled agents の install や sync は行わず、後で実装を承認した場合は .github があれば先にバックアップしてから安全に反映します。",
      "Verwenden Sie zuerst das Intro Tutorial, um das Plugin kennenzulernen. Nutzen Sie danach Plan Integration, um die vorhandenen Agenten-Oberflächen des Repositories zu prüfen und repo-lokale Skills, Prompts und Agentenrollen rund um den Todo-Kommunikations-Hub und die Scheduler-MCP-Tools zu entwerfen. Behandeln Sie bestehende repo-lokale Agentensysteme als benutzereigen, installieren oder synchronisieren Sie gebündelte Agenten erst nach ausdrücklicher Freigabe, und sichern Sie .github vorher, wenn es vorhanden ist, bevor Sie die vereinbarte Einrichtung sicher umsetzen.",
    ),
    ...
    settingsSupportBody: localize(
      "Configure repo-local Copilot MCP, skills, and starter agents here. Bundled agents only sync when you trigger it manually, and customized workspace copies are skipped so your edits stay intact.",
      "ここで repo-local の Copilot 用 MCP、スキル、スターターエージェントを設定します。同梱エージェントは手動同期のみで、ワークスペース側で変更したコピーはスキップされるため編集内容が保護されます。",
      "Konfigurieren Sie hier repo-lokales Copilot-MCP, Skills und Starter-Agenten. Gebündelte Agenten werden nur manuell synchronisiert, und angepasste Workspace-Kopien werden übersprungen, damit Ihre Änderungen erhalten bleiben.",
    ),
