...
export function buildSchedulerWebviewStrings(
  ...
  return {
    ...
    helpAgentEcosystemBody: localize(
      "Use the intro tutorial to learn the plugin first, then use plan integration to inspect the repo's current agent surfaces and design repo-local skills, prompts, and agent roles around the Todo communication hub and scheduler MCP tools. That optional layer is useful when you want the orchestrator to receive the task, delegate bounded work to the right repo-local or starter-pack specialist, and keep its own chat context cleaner because the specialist reports back after the assigned loop. Treat existing repo-local agent systems as user-owned, only approve bundled-agent install or sync when you want a manual Settings sync, back up .github first when available, and rely on customized workspace copies being skipped so the agreed setup does not overwrite repo-specific edits.",
      "まず Intro Tutorial でプラグインの全体像を理解し、その後 Plan Integration でリポジトリ内の既存エージェント構成を確認しながら、Todo コミュニケーションハブと scheduler MCP ツールを中心に repo-local のスキル、プロンプト、エージェント役割を設計します。この任意のレイヤーを使うと、orchestrator がタスクを受け取り、repo-local または starter pack の適切な specialist に制限付きの作業を委譲し、その specialist が担当ループを終えてから報告するため、メインチャットの文脈をより整理したまま保てます。既存の repo-local agent system はユーザー所有として扱い、bundled agents の install や sync を行うのは Settings からの手動同期を明示的に承認したときだけにし、.github があれば先にバックアップして、workspace 側でカスタマイズされたコピーはスキップされる前提で repo 固有の編集を上書きしないようにします。",
      "Verwenden Sie zuerst das Intro Tutorial, um das Plugin kennenzulernen. Nutzen Sie danach Plan Integration, um die vorhandenen Agenten-Oberflächen des Repositories zu prüfen und repo-lokale Skills, Prompts und Agentenrollen rund um den Todo-Kommunikations-Hub und die Scheduler-MCP-Tools zu entwerfen. Diese optionale Ebene ist nützlich, wenn der Orchestrator die Aufgabe annehmen, begrenzte Arbeit an den passenden repo-lokalen oder Starter-Pack-Spezialisten delegieren und seinen eigenen Chat-Kontext sauberer halten soll, weil der Spezialist erst nach der zugewiesenen Schleife zurückmeldet. Behandeln Sie bestehende repo-lokale Agentensysteme als benutzereigen, geben Sie die Installation oder Synchronisierung gebündelter Agenten nur dann frei, wenn Sie eine manuelle Synchronisierung aus den Einstellungen wollen, sichern Sie .github vorher, wenn es vorhanden ist, und verlassen Sie sich darauf, dass angepasste Workspace-Kopien übersprungen werden, damit repo-spezifische Änderungen nicht überschrieben werden.",
    ),
    ...
    boardApprovePrompt: localize("Mark this todo ready for task draft creation?", "この Todo を task draft 作成準備済みにしますか？", "Dieses Todo für die Erstellung eines Task-Entwurfs als bereit markieren?"),
