...
export function buildSchedulerWebviewStrings(
  ...
  return {
    ...
    helpAgentEcosystemBody: localize(
      "Use the intro tutorial to learn the plugin first, then use plan integration to inspect the repo's current agent surfaces and design repo-local skills, prompts, and agent roles around the Todo communication hub and scheduler MCP tools. Treat existing repo-local agent systems as user-owned, only approve bundled-agent install or sync when you want a manual Settings sync, back up .github first when available, and rely on customized workspace copies being skipped so the agreed setup does not overwrite repo-specific edits.",
      "まず Intro Tutorial でプラグインの全体像を理解し、その後 Plan Integration でリポジトリ内の既存エージェント構成を確認しながら、Todo コミュニケーションハブと scheduler MCP ツールを中心に repo-local のスキル、プロンプト、エージェント役割を設計します。既存の repo-local agent system はユーザー所有として扱い、bundled agents の install や sync を行うのは Settings からの手動同期を明示的に承認したときだけにし、.github があれば先にバックアップして、workspace 側でカスタマイズされたコピーはスキップされる前提で repo 固有の編集を上書きしないようにします。",
      "Verwenden Sie zuerst das Intro Tutorial, um das Plugin kennenzulernen. Nutzen Sie danach Plan Integration, um die vorhandenen Agenten-Oberflächen des Repositories zu prüfen und repo-lokale Skills, Prompts und Agentenrollen rund um den Todo-Kommunikations-Hub und die Scheduler-MCP-Tools zu entwerfen. Behandeln Sie bestehende repo-lokale Agentensysteme als benutzereigen, geben Sie die Installation oder Synchronisierung gebündelter Agenten nur dann frei, wenn Sie eine manuelle Synchronisierung aus den Einstellungen wollen, sichern Sie .github vorher, wenn es vorhanden ist, und verlassen Sie sich darauf, dass angepasste Workspace-Kopien übersprungen werden, damit repo-spezifische Änderungen nicht überschrieben werden.",
    ),
    ...
    settingsSupportBody: localize(
      "Configure repo-local Copilot MCP, skills, and starter agents here. Existing repo-local agent setups stay user-owned, bundled agents only sync when you approve a manual action, .github is backed up first when available, and customized workspace copies are skipped so your edits stay intact.",
      "ここで repo-local の Copilot 用 MCP、スキル、スターターエージェントを設定します。既存の repo-local agent setup は引き続きユーザー所有として扱われ、同梱エージェントは手動操作を承認したときだけ同期され、.github があれば先にバックアップされ、ワークスペース側で変更したコピーはスキップされるため編集内容が保護されます。",
      "Konfigurieren Sie hier repo-lokales Copilot-MCP, Skills und Starter-Agenten. Bestehende repo-lokale Agent-Setups bleiben benutzereigen, gebündelte Agenten werden nur synchronisiert, wenn Sie eine manuelle Aktion freigeben, .github wird vorher gesichert, wenn vorhanden, und angepasste Workspace-Kopien werden übersprungen, damit Ihre Änderungen erhalten bleiben.",
    ),
