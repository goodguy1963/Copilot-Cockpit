...
): { taskStorePath: string; storageMetaFilePath: string } {
  return {
    taskStorePath: path.join(globalStoragePath, storageFileName),
...
export async function persistScheduleManagerTaskStore(options: {
  ...
  writeTasksToDisk: (tasksArray: ScheduledTask[]) => Promise<void>;
  ...
  try {
    await options.writeTasksToDisk(options.tasksArray);
    ...
    void Promise.all([
      options.writeTasksToDisk(options.tasksArray),
