...
import { getResourceScopedSettingsTarget } from "../../copilotWebviewSettingsHandler";
...
function readSchedulerWebviewScriptSource(): string {
  ...
  return readSourceFilesFromDirectory(
    ["../../../media"],
    (fileName) => fileName.endsWith(".js") && fileName !== "copilotWebview.entry.js",
  );
}

function readGeneratedSchedulerWebviewScriptSource(): string {
  return readWorkspaceFile("../../../media/generated/copilotWebview.js");
}

function readSchedulerWebviewTemplateSource(): string {
  // Template/CSS markup is composed from copilotWebview* source modules.
  return readSourceFilesFromDirectory(
    ["../../../src"],
    (fileName) => fileName.startsWith("copilotWebview") && fileName.endsWith(".ts"),
  );
}

function readSchedulerWebviewStringsSource(): string {
  return readWorkspaceFile("../../../src/copilotWebviewStrings.ts");
}

function readSchedulerWebviewDebugSource(): string {
  return readWorkspaceFile("../../../media/copilotWebviewDebug.js");
}

function readBoardRenderingSource(): string {
  return readWorkspaceFile("../../../media/copilotWebviewBoardRendering.js");
}
...
function withPostedWebviewMessages<T>(
  ...
  try {
    wv.activePanel = createPostedMessagePanel(sent);
    wv.pendingMessages = [];
...
async function withRefreshCacheHarness(
  ...
  await withPostedWebviewMessages({ ready: true }, async ({ sent }) => {
    ...
    wv.refreshAgentsAndModelsCache = async () => {
      wv.agentListCache = [{ id: "agent" }];
      wv.modelListCache = [{ id: "gpt-4o" }];
    };
    wv.refreshPromptTemplatesCache = async () => {
      wv.templateCache = [{ path: "prompt.md" }];
    };
...
suite("SchedulerWebview Message Queue Behavior", () => {
  function loadBoardInteractionModule() {
    const scriptPath = resolveWorkspacePath(
      "../../../media/copilotWebviewBoardInteractions.js",
    );
  ...
  test("webview client script parses", () => {
    const scriptPath = resolveWorkspacePath(
      "../../../media/generated/copilotWebview.js",
    );
  ...
  test("task list recovers restored label filters that would otherwise hide all tasks and ready todo drafts", () => {
    ...
    expectSourceToIncludeSnippets(scriptSource, [
      ...
      'getReadyTodoDraftCandidates(activeLabelFilter).length > 0',
      'getReadyTodoDraftCandidates("").length === 0',
      'restoredLabelFilterWasExplicit = false;',
      'taskItems = recoverLabelFilterIfRestoredViewIsEmpty(taskItems);',
    ], "restored label filter recovery");
  });
  ...
  test("todo board cards use icon-only compact actions and keep board rows on one line", () => {
    const renderPath = path.resolve(
      __dirname,
      "../../../media/copilotWebviewBoardRendering.js",
    );
  ...
  test("Queues messages until panel is ready then flushes (dedup by type)", () => {
    withPostedWebviewMessages({}, ({ sent, wv }) => {
      assert.ok(typeof wv.postMessage === "function");
      assert.ok(typeof wv.drainQueuedMessages === "function");
      ...
      assert.strictEqual(queued.length, 2, "two messages queued");
      ...
      (wv.drainQueuedMessages as () => void)();

      assert.strictEqual(sent.length, 2, "two messages sent");
  ...
  test("Batches repeated update messages while ready", () => {
    withPostedWebviewMessages(
      ...
      ({ sent, wv }) => {
        ...
        assert.strictEqual(queued.length, 2, "expect two queued");
        assert.deepStrictEqual(
        ...
        assert.strictEqual(sent.length, 2, "expect two sent");
        assert.deepStrictEqual(
  ...
  test("board interaction binding reinstalls click delegation when the board root changes", () => {
    const helpers = loadBoardInteractionModule();
    const calls: string[] = [];
    const firstBoardColumns = createListenerTarget({
      contains: () => false,
      querySelectorAll: () => [],
    });
    const deleteButton = createListenerTarget({
      getAttribute: (name: string) => (name === "data-todo-delete" ? "todo-2" : ""),
      closest: (selector: string) => (selector === "[data-todo-delete]" ? deleteButton : null),
    });
    const secondBoardColumns = createListenerTarget({
      contains: (value: unknown) => value === deleteButton,
      querySelectorAll: () => [],
    });

    helpers.bindBoardColumnInteractions({
      boardColumns: firstBoardColumns,
      getBoardColumns: () => firstBoardColumns,
      document: {},
    ...
    helpers.bindBoardColumnInteractions({
      boardColumns: secondBoardColumns,
      getBoardColumns: () => secondBoardColumns,
      document: {},
      window: { addEventListener: () => undefined },
      vscode: { postMessage: () => undefined },
      renderCockpitBoard: () => undefined,
    ...
    assert.ok(typeof secondBoardColumns.listeners.click === "function");

    secondBoardColumns.listeners.click({
      target: deleteButton,
      stopPropagation: () => undefined,
      preventDefault: () => undefined,
    });

    assert.deepStrictEqual(calls, ["delete-confirm"]);
  });

  test("board interaction binding reinstalls pointer listeners when the window changes", () => {
    const helpers = loadBoardInteractionModule();
    const firstWindowListeners: Record<string, (event: Record<string, unknown>) => void> = {};
    const secondWindowListeners: Record<string, (event: Record<string, unknown>) => void> = {};
    const boardColumns = createListenerTarget({
      contains: () => false,
      querySelectorAll: () => [],
    });
    ...
    helpers.bindBoardColumnInteractions({
      ...
      window: {
        addEventListener: (name: string, handler: (event: Record<string, unknown>) => void) => {
          firstWindowListeners[name] = handler;
        },
      },
      vscode: { postMessage: () => undefined },
      renderCockpitBoard: () => undefined,
      openTodoEditor: () => undefined,
      openTodoDeleteModal: () => undefined,
      setPendingBoardDelete: () => undefined,
      clearPendingBoardDelete: () => undefined,
      submitBoardDeleteChoice: () => undefined,
      handleSectionCollapse: () => undefined,
      handleSectionRename: () => undefined,
    ...
    helpers.bindBoardColumnInteractions({
      ...
      window: {
        addEventListener: (name: string, handler: (event: Record<string, unknown>) => void) => {
          secondWindowListeners[name] = handler;
        },
      },
      vscode: { postMessage: () => undefined },
      renderCockpitBoard: () => undefined,
    ...
    assert.ok(typeof firstWindowListeners.pointermove === "function");
    assert.ok(typeof secondWindowListeners.pointermove === "function");
    assert.ok(typeof secondWindowListeners.pointerup === "function");
    assert.ok(typeof secondWindowListeners.pointercancel === "function");
  });
  ...
  test("board interaction binding uses pointer drag for todo moves", () => {
    ...
    const pointerCaptureCalls: string[] = [];
    ...
    const sectionHeader = {
      querySelector: (selector: string) => (selector === "[data-section-collapse]" ? sectionCollapseButton : null),
      closest: (selector: string) => {
        if (selector === ".cockpit-section-header") return sectionHeader;
        if (selector === "[data-section-id]") return section;
        return null;
    ...
    const draggedCard = createListenerTarget({
      ...
      setPointerCapture: (pointerId: number) => pointerCaptureCalls.push(`set:${pointerId}`),
      releasePointerCapture: (pointerId: number) => pointerCaptureCalls.push(`release:${pointerId}`),
    });
    const dragHandle = {
    ...
    const boardColumns = createListenerTarget({
      contains: (value: unknown) => value === dragHandle || value === draggedCard || value === dropCard || value === section || value === sectionHeader || value === sectionCollapseButton,
      querySelectorAll: (selector: string) => {
        if (selector === "[data-section-id]") return [section];
        if (selector === "[data-todo-id]") return [draggedCard];
        return [];
    ...
    helpers.bindBoardColumnInteractions({
      ...
      openTodoEditor: () => undefined,
      openTodoDeleteModal: () => undefined,
      handleSectionCollapse: () => boardCalls.push("collapse"),
      handleSectionRename: () => undefined,
      handleSectionDelete: () => undefined,
      handleTodoCompletion: () => undefined,
      setSelectedTodoId: () => undefined,
    ...
    documentListeners.mouseup({ clientX: 20, clientY: 30 });
    ...
    assert.deepStrictEqual(boardCalls, []);
  ...
  test("board interaction binding completes todo drop on lost pointer capture using the last drag target", () => {
    ...
    assert.strictEqual(draggingTodoId, "todo-1");
    assert.strictEqual(isBoardDragging, true);
    assert.ok(typeof dragHandle.listeners.lostpointercapture === "function");

    dragHandle.listeners.lostpointercapture({ pointerId: 7 });
  ...
  test("board interaction binding cancels stale drag state on window blur", () => {
    const helpers = loadBoardInteractionModule();
    const windowListeners: Record<string, (event: Record<string, unknown>) => void> = {};
    let draggingTodoId: string | null = null;
    let isBoardDragging = false;
    ...
    const boardColumns = createListenerTarget({
      contains: (value: unknown) => value === dragHandle || value === draggedCard || value === section,
      querySelectorAll: (selector: string) => {
        if (selector === "[data-section-id]") return [section];
        if (selector === "[data-todo-id]") return [draggedCard];
        return [];
    ...
    assert.strictEqual(draggingTodoId, "todo-1");
    assert.strictEqual(isBoardDragging, true);
    assert.ok(typeof windowListeners.blur === "function");
  ...
  test("board interaction binding computes same-section drop index after excluding the dragged todo", () => {
    const helpers = loadBoardInteractionModule();
    const postedMessages: Array<Record<string, unknown>> = [];
    const windowListeners: Record<string, (event: Record<string, unknown>) => void> = {};
    let draggingTodoId: string | null = null;
    ...
    windowListeners.pointermove({ clientX: 22, clientY: 22 });
    windowListeners.pointerup({ clientX: 22, clientY: 22 });
  ...
  test("board interaction binding starts section drag from header body after movement threshold", () => {
    const helpers = loadBoardInteractionModule();
    const postedMessages: Array<Record<string, unknown>> = [];
    const boardCalls: string[] = [];
    const windowListeners: Record<string, (event: Record<string, unknown>) => void> = {};
    ...
    const boardColumns = createListenerTarget({
      contains: (value: unknown) => value === sectionHeader || value === headerTitle || value === draggedSection || value === dropSection || value === sectionCollapseButton,
      querySelectorAll: (selector: string) => {
        if (selector === ".cockpit-section-header") return [sectionHeader];
        ...
        if (selector === "[data-section-id]") return [draggedSection, dropSection];
        if (selector === "[data-section-id].section-drag-over") return [];
        if (selector === "[data-section-id].section-dragging") return [];
        if (selector === "[data-todo-id].todo-dragging") return [];
        if (selector === "[data-todo-id].todo-drop-target") return [];
    ...
    assert.ok(typeof sectionHeader.listeners.pointerdown === "function");

    sectionHeader.listeners.pointerdown({
      button: 0,
      clientX: 10,
      clientY: 10,
      target: headerTitle,
...
suite("SchedulerWebview Jobs Request Tests", () => {
  ...
  test("requestDeleteJobTask can detach from workflow without deleting the task", async () => {
    ...
    const savedTasks = wv.currentTasks;
    const originalShowWarningMessage = (vscode.window as any).showWarningMessage;
    const actions: unknown[] = [];
  ...
  test("requestDeleteJobTask can delete the task entirely", async () => {
    ...
    const originalAction = wv.onTaskActionCallback;
    const originalJobs = wv.currentJobs;
    const snapshotTasks = wv.currentTasks;
    const originalShowWarningMessage = (vscode.window as any).showWarningMessage;
    const actions: unknown[] = [];
  ...
  test("testPrompt invokes the configured callback", async () => {
    ...
    const priorCallback = wv.onTestPromptCallback;
    ...
    try {
      ...
      wv.onTestPromptCallback = priorCallback;
    }
  });
...
suite("SchedulerWebview Error Detail Sanitization", () => {
  test("Strips absolute paths to basenames on Windows and POSIX", () => {
    const wv = SchedulerWebview as unknown as RefreshableSchedulerWebviewInternals;

    assert.ok(typeof wv.redactPathsForDisplay === "function");

    const sanitize = wv.redactPathsForDisplay!;
...
suite("SchedulerWebview showError Path Sanitization", () => {
  ...
  test("showError strips abs paths before posting to webview", () => {
    withPostedWebviewMessages({ ready: true }, ({ sent }) => {
      SchedulerWebview.showError( // sanitize-test
        "ENOENT: no such file or directory, open 'C:\\Users\\me\\secret folder\\a b.md'",
      );
...
suite("Board Render State Tests", () => {
  function loadBoardStateModule() {
    const scriptPath = resolveWorkspacePath(
      "../../../media/copilotWebviewBoardState.js",
    );
    const scriptSource = fs
      .readFileSync(scriptPath, "utf8")
      .replace(/^export\s+/gm, "");
    const context = vm.createContext({ result: undefined });
    const moduleScript = new vm.Script(
      `${scriptSource}\nresult = { createBoardRenderState, requestBoardRender, finishBoardDrag };`,
      { filename: scriptPath },
    );
    moduleScript.runInContext(context);
    return context.result as {
      createBoardRenderState: () => Record<string, unknown>;
      requestBoardRender: (
      ...
      finishBoardDrag: (
        state: Record<string, unknown>,
        resetSectionDragState: () => void,
        requestRender: () => void,
      ) => void;
    };
  }

  test("second render request after first rAF fires schedules a new frame", () => {
    const { createBoardRenderState, requestBoardRender } = loadBoardStateModule();
    const state = createBoardRenderState();
    let renderCount = 0;
    ...
    const renderBoard = () => { renderCount++; };

    // First request schedules a frame
    requestBoardRender(state, fakeRaf, renderBoard);
    assert.strictEqual(pendingCallbacks.length, 1, "first call should schedule a frame");

    // Fire the first rAF
    pendingCallbacks[0]();
    assert.strictEqual(renderCount, 1, "first frame should render");
    assert.strictEqual(state.scheduledBoardRenderFrame, 0, "frame ID should be cleared");

    // Second request must still schedule a new frame
    requestBoardRender(state, fakeRaf, renderBoard);
    assert.strictEqual(pendingCallbacks.length, 2, "second call must schedule a new frame");

    // Fire the second rAF
    pendingCallbacks[1]();
    assert.strictEqual(renderCount, 2, "second frame should render");
  });

  test("finishBoardDrag flushes pending render that was deferred during drag", () => {
    ...
    // Start dragging
    state.isBoardDragging = true;

    // Render request while dragging should be deferred
    requestBoardRender(state, fakeRaf, renderBoard);
    assert.strictEqual(pendingCallbacks.length, 0, "no frame scheduled while dragging");
    assert.strictEqual(state.pendingBoardRender, true, "render should be pending");

    // Finish the drag
    let finishRenderCalled = false;
    finishBoardDrag(
      state,
      () => {},
      () => { finishRenderCalled = true; },
    );
