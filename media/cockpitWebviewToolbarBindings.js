...
export function bindRestoreHistoryButton(button, options) {
  bindClickAction(button, function () {
    var snapshotId = options.cockpitHistorySelect
      ? options.cockpitHistorySelect.value
      : "";
    if (!snapshotId) {
      options.window.alert(
        options.strings.cockpitHistoryRestoreSelectRequired ||
    ...
    var confirmText = (
      options.strings.cockpitHistoryRestoreConfirm ||
