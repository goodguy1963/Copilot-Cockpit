...
(() => {
  ...
  (function() {
    ...
    bindClickAction(needsBotReviewPromptRecommendedBtn, function() {
      ...
      renderReviewPromptRecommendationStatus();
      if (reviewDefaultsNote) {
        reviewDefaultsNote.textContent = strings.reviewDefaultsRecommendedLoaded || "Recommended prompt text loaded. Save to persist it.";
      }
    });
    ...
    function renderAgentModelsControls() {
      ...
      for (var ai = 0; ai < localAgents.length; ai++) {
        ...
        rows.push(
          '<div class="agent-model-row" style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--vscode-panel-border);"><strong style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml(agentName) + '</strong><select class="agent-model-select" data-agent-id="' + agentIdAttr + '" data-agent-filepath="' + filePathAttr + '" style="max-width:240px;">' + opts + "</select></div>"
        );
      }
