...
(() => {
  ...
  function createSchedulerWebviewDomRefs(document2) {
    return {
      ...
      settingsDownloadLatestBtn: document2.getElementById("settings-download-latest-btn"),
      ...
      if (refs.settingsUpdateStatusRow) {
        if (view.hasNewVersion) {
          ...
          if (refs.settingsUpdateStatusText) {
            refs.settingsUpdateStatusText.textContent = strings.settingsUpdateAvailable ? strings.settingsUpdateAvailable + " (" + view.latestStableVersion + ")" : "Update available (" + view.latestStableVersion + ")";
