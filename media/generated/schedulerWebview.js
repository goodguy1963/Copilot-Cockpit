...
(() => {
  ...
  function renderTodoCompactActions(card, options) {
    ...
    return '<div class="' + actionClassName + '"><button type="button" class="btn-secondary todo-card-edit todo-list-action-btn" data-todo-edit="' + helpers.escapeAttr(card.id) + '" title="' + helpers.escapeAttr(strings.boardEditTodo || "Open Editor") + '" style="color:var(--vscode-textLink-foreground);">&#9998; ' + helpers.escapeHtml(strings.boardEditTodoShort || "Edit") + "</button>" + (canDelete ? '<button type="button" class="btn-secondary todo-card-delete todo-list-action-btn" data-todo-delete="' + helpers.escapeAttr(card.id) + '" title="' + helpers.escapeAttr(strings.boardDeleteTodo || "Delete Todo") + '">&#128465; ' + helpers.escapeHtml(strings.boardDeleteTodoShort || "Delete") + "</button>" : "") + "</div>";
  }
  ...
  (function() {
    ...
    function renderTodoCompletionButton(card) {
      var title = card && card.archived ? strings.boardCompleteTodo || "Complete and archive" : getTodoCompletionActionLabel(card);
      var icon = isTodoCompleted(card) ? "\u2713" : isTodoReadyForFinalize(card) ? "\u2713\u2713" : "\u25CB";
      return '<button type="button" class="todo-complete-button" data-todo-complete="' + escapeAttr(card.id) + '" data-no-drag="1" title="' + escapeAttr(title) + '" aria-label="' + escapeAttr(title) + '" ' + (card.archived ? "disabled " : "") + 'style="display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:28px;border-radius:999px;border:1px solid var(--vscode-input-border, var(--vscode-panel-border));background:' + (isTodoCompleted(card) ? "var(--vscode-button-background)" : "var(--vscode-input-background)") + ";color:" + (isTodoCompleted(card) ? "var(--vscode-button-foreground)" : "var(--vscode-foreground)") + ';cursor:pointer;font-size:12px;font-weight:700;line-height:1;flex:0 0 auto;"><span aria-hidden="true">' + escapeHtml(icon) + "</span></button>";
    }
    ...
    function renderTodoDetailPanel(selectedTodo, sections) {
      ...
      if (todoCommentList) {
        ...
        todoCommentList.innerHTML = comments.length > 0 ? comments.map(function(comment) {
          ...
          return '<article style="padding:10px;border-radius:8px;border:1px solid var(--vscode-panel-border);background:var(--vscode-sideBar-background);"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:6px;"><strong>#' + escapeHtml(String(sequence)) + " \u2022 " + escapeHtml(sourceLabel) + '</strong><span class="note">' + escapeHtml(formatTodoDate(displayDate)) + '</span></div><div class="note" style="margin-bottom:6px;">' + escapeHtml(comment.author || "system") + '</div><div class="note" style="white-space:pre-wrap;">' + escapeHtml(comment.body || "") + "</div></article>";
    ...
    function renderCockpitBoard() {
      ...
      if (todoLabelSuggestions) {
        todoLabelSuggestions.onclick = function(event) {
          var btn = event.target && event.target.closest ? event.target.closest("[data-label-suggestion]") : null;
          if (btn) {
    ...
    function syncEditorTabLabels() {
      setEditorTabLabel(
        "create",
        editingTaskId ? strings.tabTaskEditorEdit || strings.tabEdit || "Edit Task" : strings.tabTaskEditorCreate || strings.tabTaskEditor || "Create Task"
      );
      setEditorTabLabel(
        "todo-edit",
        selectedTodoId ? strings.tabTodoEditorEdit || strings.boardDetailTitleEdit || "Edit Todo" : strings.tabTodoEditorCreate || strings.tabTodoEditor || "Create Todo"
      );
      setEditorTabLabel(
        "jobs-edit",
        isCreatingJob || !selectedJobId ? strings.tabJobsEditorCreate || strings.tabJobsEditor || "Create Job" : strings.tabJobsEditorEdit || "Edit Job"
      );
    }
    ...
    function renderJobsTab() {
      ...
      if (jobsWorkflowMetrics) {
        jobsWorkflowMetrics.innerHTML = [
          ...
          return '<div class="jobs-workflow-metric' + (metric.tone ? " " + metric.tone : "") + '" title="' + escapeAttr(metric.value) + '"><div class="jobs-workflow-metric-label">' + escapeHtml(metric.label) + '</div><div class="jobs-workflow-metric-value"' + (metric.valueAttr || "") + ">" + escapeHtml(metric.value) + "</div></div>";
        }).join("");
      }
      ...
      try {
        switch (message.type) {
          ...
          case "focusJob":
            ...
            openJobEditor(selectedJobId);
            setTimeout(function() {
              var jobCard = selectedJobId ? document.querySelector('[data-job-id="' + selectedJobId + '"]') : null;
