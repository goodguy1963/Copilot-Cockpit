...
(() => {
  ...
  function renderTodoListRow(card, sectionId, options) {
    ...
    var chipMarkup = cardFlag || visibleLabels.length ? '<div class="todo-list-chip-row">' + (cardFlag ? helpers.renderFlagChip(cardFlag, false) : "") + (visibleLabels.length ? '<div class="card-labels">' + visibleLabels.map(function(label, idx) {
  ...
  (function() {
    ...
    function isProtectedFlagDefinition(entryOrName) {
      ...
      return key === "go" || key === "rejected" || key === "abgelehnt" || key === "needs-bot-review" || key === "needs-user-review" || key === "new";
    }
    ...
    function renderTodoDetailPanel(selectedTodo, sections) {
      ...
      if (todoCommentList) {
        ...
        todoCommentList.innerHTML = comments.length > 0 ? comments.map(function(comment, commentIndex) {
          ...
          return '<article class="todo-comment-card' + toneClass + userFormClass + '" data-comment-index="' + escapeAttr(String(commentIndex)) + '" tabindex="0" role="button" aria-label="' + escapeAttr(strings.boardCommentOpenFull || "Open full comment") + '"><div class="todo-comment-header"><strong>#' + escapeHtml(String(sequence)) + " \u2022 " + escapeHtml(sourceLabel) + '</strong><div style="display:flex;align-items:center;gap:6px;"><span class="note">' + escapeHtml(formatTodoDate(displayDate)) + '</span><button type="button" class="btn-icon todo-comment-delete-btn" data-delete-comment-index="' + escapeAttr(String(commentIndex)) + '" title="' + escapeAttr(strings.boardCommentDelete || "Delete comment") + '">&#128465;</button></div></div><div class="note todo-comment-author">' + escapeHtml(comment.author || "system") + '</div><div class="note todo-comment-body">' + escapeHtml(comment.body || "") + '</div><div class="todo-comment-expand-hint">' + escapeHtml(strings.boardCommentOpenFull || "Open full comment") + "</div></article>";
