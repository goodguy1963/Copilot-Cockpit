...
Copilot Cockpit is a repo-local orchestration layer for AI work in VS Code. It separates planning, approval, execution, and review so users keep control while agents still get structure, tooling, and automation support.
...
Codex support is currently limited. It can help create and coordinate todos and task drafts, but scheduled task execution does not run through Codex today. Tasks run through GitHub Copilot in VS Code or the extension's OpenRouter-backed native chat path. Scheduling tasks directly through the Codex VS Code extension is not implemented yet.
